...
Repository-hosted tooling recovers the `ml.extra` overlay and validates that the checked-in stub matches the archival bytes. Run `ml-extra-refresh --metadata-json docs/porting/artifacts/ml-extra-overlay-metadata.json --baseline docs/porting/artifacts/ml-extra-overlay-metadata.json --if-changed` after any overlay rescan to export a fresh snapshot, diff it against the committed baseline, and receive CI-friendly exit codes. The helper shell wrapper at `tools/ml-extra-refresh.sh` pins those defaults for local automation, while `python -m imagebbs.ml_extra_refresh_pipeline` remains available for module-centric workflows that need to customise the invocation further. For manual checks, `python -m imagebbs.ml_extra_snapshot_guard --baseline docs/porting/artifacts/ml-extra-overlay-metadata.json` still emits the human-readable summary, while the new `--json` and `--update-baseline` options provide machine-readable diffs and opt-in baseline refreshes for CI workflows.【F:src/imagebbs/ml_extra_refresh_pipeline.py†L1-L12】【F:src/imagebbs/ml_extra_snapshot_guard.py†L1-L12】【F:tools/ml-extra-refresh.sh†L1-L17】

Run `python -m imagebbs.ml_extra_dump_macros --hashes-json docs/porting/artifacts/ml-extra-macro-hashes.json` to persist a compact slot-to-hash snapshot stamped with the overlay path and capture time. Commit the refreshed JSON whenever macro payload bytes change so the repository’s artefacts reflect the latest overlay state.
