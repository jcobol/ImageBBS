...
def test_default_rom_path_used_when_present(tmp_path) -> None:
    rom_path = tmp_path / "c64.bin"
    rom_bytes = bytes((index % 256 for index in range(4096)))
    rom_path.write_bytes(rom_bytes)

    original_path = petscii_glyphs._DEFAULT_ROM_PATH
    petscii_glyphs._DEFAULT_ROM_PATH = rom_path
    try:
        petscii_glyphs.reset_character_rom()
        glyph = petscii_glyphs.get_glyph(0x00)
        assert _rows_from_matrix(glyph) == tuple(rom_bytes[:8])
    finally:
        petscii_glyphs._DEFAULT_ROM_PATH = original_path
        petscii_glyphs._CHAR_ROM = b""
        petscii_glyphs.reset_character_rom()
