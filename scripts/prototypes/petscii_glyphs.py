...
_DEFAULT_ROM_PATH = Path(__file__).with_name("c64.bin")
_CHAR_ROM: bytes = b""
...
def _ensure_rom() -> None:
    global _CHAR_ROM
    if not _CHAR_ROM:
        if _DEFAULT_ROM_PATH.exists():
            load_character_rom(_DEFAULT_ROM_PATH)
        else:
...
def load_character_rom(payload: bytes | bytearray | memoryview | Iterable[int] | str | Path) -> None:
    ...
    if isinstance(payload, (str, Path)):
        data = Path(payload).expanduser().read_bytes()
    else:
        data = bytes(payload)
