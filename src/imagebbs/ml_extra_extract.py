...
class PointerDirectoryEntry:
    """Single pointer-directory record exported by ``ml.extra``."""
...
def load_prg(path: Path) -> tuple[int, list[int]]:
    """Return the load address and payload bytes for ``path``."""
...
def runtime_to_index(address: int, *, load_addr: int) -> int:
    """Map a runtime ``address`` to the corresponding payload index."""
...
def decode_petscii(data: Iterable[int]) -> str:
    """Return a printable PETSCII representation for ``data``."""
...
def iter_pointer_directory(
    memory: list[int], *, load_addr: int
) -> Iterator[PointerDirectoryEntry]:
    """Yield :class:`PointerDirectoryEntry` objects from ``memory``."""
...
def extract_overlay(path: Path) -> dict[str, object]:
    """Return the pointer directory recovered from the overlay PRG."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Return the command-line arguments for the extraction CLI."""
...
def main(argv: list[str] | None = None) -> None:
    """Console script entry point for the extraction helper."""
