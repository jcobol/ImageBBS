...
This module mirrors the historical prototype implementations so that runtime
code can consume pointer-directory records (macro slots, relocation addresses,
and raw PETSCII payloads) without depending on ``scripts``.  The helper resolves
the archived binary from ``v1.2/from-floppy/ml.extra`` by default and exposes
dataclasses for each recovered table, including the lightbar defaults and editor
palette stored near the overlay's tail section.
"""
...
class MLExtraDefaults:
    ...
    def from_overlay(cls, overlay_path: Optional[Path] = None) -> "MLExtraDefaults":
        ...
        palette_raw = _slice_memory(memory, load_address, _PALETTE_ADDR, _PALETTE_LENGTH)
        palette = EditorPalette(colours=tuple(palette_raw))
    ...
    def from_resources(
        cls,
        resource: str = "v1.2/from-floppy/ml.extra",
        package: str = "imagebbs",
    ) -> "MLExtraDefaults":
        """Load ``ml.extra`` from package resources and decode its tables."""

        candidate = importlib.resources.files(package).joinpath(resource)
        with importlib.resources.as_file(candidate) as resolved:
            return cls.from_overlay(resolved)
...
def _decode_flag_dispatch(
    ...
    for offset in range(1, slot_count + 1):
        flag = flag_indices[offset]
        slot = slot_ids[offset - 1]
        handler = handlers[offset - 1]
        entries.append(
            FlagDispatchEntry(flag_index=flag, slot=slot, handler_address=handler)
        )
