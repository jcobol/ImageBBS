...
- [x] Catalogue the BASIC staging routines that refill `tempbott+40`/`var_4078` so the host can cache the masked-pane payload before `outscn` consumes it. *(Iteration 47 nails down the macro-driven producers—confirming `setup` never writes the buffers—and records the implicit `&,50` flushes.)*【F:docs/porting/iteration-47.md†L9-L20】
