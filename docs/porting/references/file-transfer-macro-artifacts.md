...
- The `ml-extra-macro-screens.json` entry for slot `42` (hex `$2A`) mirrors that payload on row 0 with glyph bytes `3F 3F 20 D5 CE CB CE CF D7 CE 20 C3 CF CD CD C1 CE C4 00`. Foreground colour `$0A` spans the visible text while the null terminator drops to `$00`; subsequent cells on the row continue the `$20`/`$0A` padding to keep the buffer aligned.【F:docs/porting/artifacts/ml-extra-macro-screens.json†L1-L1】
