...
- [ ] **Ingest overlay metadata** – Load `MLExtraDefaults.from_overlay()` to obtain the palette tuple, lightbar defaults, PETSCII payloads, and VIC register write sequences; map these into host-side colour IDs and glyph definitions before initialising the renderer.【F:scripts/prototypes/ml_extra_defaults.py†L243-L301】
