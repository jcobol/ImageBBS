...
Run `python -m imagebbs.ml_extra_screen_dumps` from the repository root to regenerate `tmp/ml-extra/ml-extra-macro-screens.json` and `docs/porting/artifacts/ml-extra-flag-screens.json`. The command replays the current overlay defaults through the console renderer so the JSON captures match the glyph, colour, and PETSCII behaviour encoded in `ml.extra`. After validating the macro capture, gzip it and base64-encode the archive into `docs/porting/artifacts/ml-extra-macro-screens.json.gz.base64` so the repository keeps a text-friendly representation of the compact snapshot.
